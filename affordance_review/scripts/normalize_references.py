...
def normalize_display_title(title: Optional[str]) -> tuple[str, list[str]]:
    ...
    # Strip DOI/URL debris before punctuation flattening to keep title readable.
...
def strip_source_tail(text: str) -> str:
    """Remove venue/container tails conservatively.

    Important: keep punctuation for splitting first; normalize later.
    """
    if not text:
        return ""
    ...
    for part in parts[1:]:
        part_l = part.lower()
        part_norm = normalize_text(part_l)
        venue_norm = normalize_venue_equivalence(part_l)
        has_venue_signal = bool(PROCEEDINGS_RE.search(part_l) or VENUE_TAIL_RE.search(part_l) or venue_norm != part_norm)
        has_meta_signal = bool(
            re.search(r"\b(?:vol\.?|volume|pp?\.?|issue|no\.?)\b", part_l, re.I)
            or ISSUE_DEBRIS_RE.search(part_l)
            or PAGE_DEBRIS_RE.search(part_l)
            or NUMERIC_SEGMENT_RE.fullmatch(part_norm)
            or re.fullmatch(r"\(?\d{4}\)?", part_norm)
        )
        has_author_signal = bool(AUTHOR_LEAK_RE.search(part_l))
        if has_venue_signal or has_meta_signal or has_author_signal or TRAILING_SOURCE_FRAGMENT_RE.fullmatch(part_l):
            break
        kept.append(part)

    candidate = ", ".join(kept).strip(" ,;:").lower()
...
def normalize_venue_equivalence(text: str) -> str:
    """Whole-string canonicalization only for known venue variants."""
    ...
    normalized = WS_REGEX.sub(" ", normalized).strip()
    normalized = re.sub(r"(?:[,;:\s]+\(?\d{1,4}\)?)+$", "", normalized).strip()
...
def normalize_work_title(title: Optional[str]) -> tuple[str, list[str]]:
    """Aggressive but conservative work-level normalization for clustering keys."""
    if not title:
        return "", []
    ...
    # Normalize malformed proceedings/container strings conservatively.
    text = re.sub(r"\bprocessings\b", "proceedings", text)
    text = re.sub(r"\bproceeding\s+of\b", "proceedings of", text)
    text = re.sub(r"\bproc\.?\s*sigchi\b", "proceedings sigchi conference", text)
    text = re.sub(r"\bproceedings\s+sigchi\s+conference(?:\s+on)?\b", "proceedings sigchi conference", text)
    ...
    text = TRAILING_VENUE_NUMERIC_RE.sub(" ", text)
    text = TRAILING_EDITION_RE.sub(" ", text)
    text = re.sub(r"\b(?:classic edition|revised and expanded edition|(?:spring|summer|fall|autumn|winter)\s+\d{4}\s+edition)\b", " ", text)
    text = VOLUME_MARKER_RE.sub(" ", text)
    text = PAGE_DEBRIS_RE.sub(" ", text)
    text = ISSUE_DEBRIS_RE.sub(" ", text)
    text = PROCEEDINGS_RE.sub(" ", text)
...
def sanitize_canonical_cluster_label(title: Optional[str]) -> str:
    """Ensure cluster canonical labels are readable and not metadata-contaminated."""
    cleaned, _ = normalize_canonical_label(title)
    if not cleaned:
        return ""
    if AUTHOR_LISTISH_RE.match(cleaned) or CONTAINER_ONLY_RE.match(cleaned):
        return ""
    if DOI_REGEX.search(cleaned) or SOURCE_META_RE.search(cleaned):
        return ""
    if re.search(r"\bpp?\.?\s*\w*\d", cleaned, re.I):
        return ""
    return cleaned
...
def materialize_clusters(
    ...
    for root, members in clusters.items():
        ...
        canonical_title = sanitize_canonical_cluster_label(canonical.raw_title or canonical.title_display_norm)
        ...
        if not canonical_title or AUTHOR_LISTISH_RE.match(canonical_title) or CONTAINER_ONLY_RE.match(canonical_title) or SOURCE_META_RE.search(canonical_title):
            for alt in sorted(members, key=lambda r: title_noise_score(r.raw_title)):
                alt_title = sanitize_canonical_cluster_label(alt.raw_title or alt.title_display_norm)
                if alt_title:
